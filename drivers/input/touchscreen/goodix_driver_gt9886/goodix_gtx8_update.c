		...
		const struct firmware_info *fw_info)
{
	...
	if (fw_ver.valid) {
		...
		ts_info("Fimware    VID:0x%02X 0x%02x 0x%02x 0x%02x", fw_info->fw_vid[0], fw_info->fw_vid[1], fw_info->fw_vid[2], fw_info->fw_vid[2]);
		...
		struct goodix_ext_module *module)
{
	...
	if (ts_bdata && ts_bdata->fw_name){
		...
		ts_info("can't find goodix firmware,ues default name\n");
	}
