...
{
	int r, err;
	r = of_property_read_u32(node, "goodix,panel-max-id", &board_data->panel_max_id);
	if (r) {
		err = -ENOENT;
	} else {
	...
	r = of_property_read_u32(node, "goodix,panel-max-x", &board_data->panel_max_x);
	if (r)
		err = -ENOENT;

	r = of_property_read_u32(node, "goodix,panel-max-y", &board_data->panel_max_y);
	if (r)
		err = -ENOENT;

	r = of_property_read_u32(node, "goodix,panel-max-w", &board_data->panel_max_w);
	if (r)
		err = -ENOENT;

	r = of_property_read_u32(node, "goodix,panel-max-p", &board_data->panel_max_p);
	if (r)
		err = -ENOENT;
 ...
 * return: 0 - no error, <0 error
*/
static int goodix_parse_dt(struct device_node *node, struct goodix_ts_board_data *board_data)
{
	...
	r = of_property_read_string(node, "goodix,cfg-name", &board_data->cfg_bin_name);
	if (r) {
		ts_err("can't get cfg name");
		board_data->cfg_bin_name = NULL;
	}

	r = of_property_read_u32(node, "goodix,power-on-delay-us", &board_data->power_on_delay_us);
	if (!r) {
	...
	r = of_property_read_u32(node, "goodix,power-off-delay-us", &board_data->power_off_delay_us);
	if (!r) {
		/* 1000ms is too large, maybe you have pass
		 * a wrong value */
				...
				unsigned int sensor_id)
{
	...
	r = of_property_read_string(node, "goodix,fw-name", &board_data->fw_name);
	if (r) {
		ts_err("can't get firmware name");
		board_data->fw_name = NULL;
	}

	r = of_property_read_string(node, "goodix,limit-name", &board_data->limit_name);
	if (r) {
		...
		unsigned char *data, unsigned int len)
{
	...
	if (dev->ic_type == IC_TYPE_NORMANDY) {
		if (goodix_set_i2c_doze_mode(dev, false) != 0)
			ts_err("gtx8 i2c write:0x%04x ERROR, disable doze mode FAILED", reg);
	}
	r = goodix_i2c_write_trans(dev, reg, data, len);
	if (dev->ic_type == IC_TYPE_NORMANDY) {
		if (goodix_set_i2c_doze_mode(dev, true) != 0)
			ts_err("gtx8 i2c write:0x%04x ERROR, enable doze mode FAILED", reg);
	}
	return r;
}
	...
	unsigned char *data, unsigned int len)
{
	...
	if (dev->ic_type == IC_TYPE_NORMANDY) {
		if (goodix_set_i2c_doze_mode(dev, false) != 0)
			ts_err("gtx8 i2c read:0x%04x ERROR, disable doze mode FAILED", reg);
	}
	r = goodix_i2c_read_trans(dev, reg, data, len);
	if (dev->ic_type == IC_TYPE_NORMANDY) {
		if (goodix_set_i2c_doze_mode(dev, true) != 0)
			ts_err("gtx8 i2c read:0x%04x ERROR, enable doze mode FAILED", reg);
	}
	return r;
}
...
static void goodix_cmds_init(struct goodix_ts_cmd *ts_cmd, u8 cmds, u8 cmd_data, u32 reg_addr)
{
	if (reg_addr) {
