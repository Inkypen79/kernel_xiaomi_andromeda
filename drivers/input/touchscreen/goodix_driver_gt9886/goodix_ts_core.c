...
{	pr_err("enter goodix_ts_sysfs_init\n");
		...
		struct goodix_touch_data *touch_data)
{
	...
	for (i = 0; i < ts_bdata->panel_max_id * 2; i++) {
		if (touch_num && i == id) { /* this is a valid touch down event */
			...
			/*if (core_data->event_status == 0x08)
				input_report_abs(dev, ABS_MT_TOUCH_MAJOR, coords->overlapping_area);*/

			/*ts_debug("report:[%d](%d, %d, %d, %d)", touch_data->coords[0].id,
	...
	if (core_data->event_status == 0x88 && core_data->fod_status) {
				...
				size_t count, loff_t *pos)
{
	...
	if (*pos != 0 || !goodix_core_data)
		return 0;
	ret = goodix_get_lockdowninfo(goodix_core_data);
	if (ret) {
