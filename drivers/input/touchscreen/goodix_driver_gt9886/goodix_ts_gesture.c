...
extern int goodix_ts_resume(struct goodix_ts_core *core_data);
 ...
 */
static ssize_t gsx_gesture_type_show(struct goodix_ext_module *module, char *buf)
{
	int count = 0;
	int i;
	int ret = 0;
...
static ssize_t gsx_gesture_enable_show(struct goodix_ext_module *module, char *buf)
{
...
{
	if (enable &&
			(atomic_read(&goodix_core_data->suspend_stat) == TP_SLEEP)) {
 ...
 */
static ssize_t gsx_gesture_data_show(struct goodix_ext_module *module, char *buf)
{
...
const struct goodix_ext_attribute gesture_attrs[] = {
	...
	__EXTMOD_ATTR(enable, 0666, gsx_gesture_enable_show, gsx_gesture_enable_store),
	...
	struct goodix_ext_module *module)
{
	...
	if (core_data->fod_status) {
		if ((FP_Event_Gesture == 1) && (temp_data[2] == 0x46)) {
			...
			input_report_abs(core_data->input_dev, ABS_MT_TOUCH_MAJOR, overlapping_area);
	...
	struct goodix_ext_module *module)
{
	...
	if (core_data->double_wakeup) {
		...
		ts_info("Set IC double wakeup mode on");
	} else {
		state_data[0] = GSX_GESTURE_CMD;
		state_data[1] = 0x00;
		state_data[2] = 0xF8;
		ret = goodix_i2c_write(dev, GSX_REG_GESTURE, state_data, 3);
		ts_info("Set IC double wakeup mode off");
	}

	goodix_i2c_read(dev, GSX_REG_GESTURE, state_data_confirm, 3);
	ts_info("read last cmd stata %x", state_data_confirm[0]);

	while (i != 0 && (GSX_GESTURE_CMD == state_data_confirm[0])) {
		i--;
		goodix_i2c_read(dev, GSX_REG_GESTURE, state_data_confirm, 3);
	}
	...
	ts_info("i == %d", i);
	if (core_data->fod_status) {
		state_data[0] = GSX_FOD_CMD;
		state_data[1] = 0x00;
		state_data[2] = 0xE7;
		goodix_i2c_write(dev, GSX_REG_GESTURE, state_data, 3);
		ts_info("Set IC fod mode on");
	} else {
		state_data[0] = GSX_FOD_CMD;
		state_data[1] = 0x01;
		state_data[2] = 0xE6;
		goodix_i2c_write(dev, GSX_REG_GESTURE, state_data, 3);
		ts_info("Set IC fod mode off");
	}
