...
static ssize_t nvt_edge_reject_switch_proc_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	*/
	...
	cnt = snprintf(buf, PAGE_SIZE - len, "edge_reject_switch: %d ", edge_reject_switch);
	buf += cnt;
	len += cnt;
...
int32_t nvt_extra_proc_init(void)
{
	...
	if (NVT_proc_edge_reject_switch_entry == NULL) {
		NVT_ERR("create proc/nvt_edge_reject_switch Failed!\n");
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/nvt_edge_reject_switch Succeeded!\n");
	}
